...
We're optimizing for simplicity here. The language balances conventions from
other languages with choices to not overcomplicate things. Not overloading
operators, for example. Mostly we want to have "good error messages" which is
...
but only cosnsumes a single newline after trailing delimeter, leaving further
consumption to pStatement.
