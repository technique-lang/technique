...
{- |
Environment in the type-theory sense of the word: the map(s) between names and
their bindings.
-}
...
{- |
Take a translator action and an environment and spin it up into a Step or nest
of Steps ("Subroutine") suitable for interpretation. In other words, translate
between the concrete syntax types and the abstract syntax we can feed to an
evaluator.
-}
...
{- |
Blocks are scoping mechanisms, so accumulated environment is discarded once we
finish resolving names within it.
-}
...
{- |
A given procedure call can either be to a user declared in-scope procedure or
to a primative builtin. We have Invocation as the Step constructors for these
cases.
-}
...
{- |
Identifiers are valid names but Names are unique, so that we can put them into
the environment map. This is where we check for reuse of an already declared
name (TODO) and given the local use of the identifier a scope-local (or
...
{- |
This begins a new (more refined) scope and does *not* add its declarations or
...
{- |
The second stage of translation phase: iterate through the Steps and where a
function call is made, look up to see if we actually know what it is.
-}
...
need to generate an error message we can offer one with position information.
-}
